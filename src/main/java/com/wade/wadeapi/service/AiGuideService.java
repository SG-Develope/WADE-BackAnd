...
public class AiGuideService {
    ...
    public AiGuideResponse getSafetyGuide(Map<String, Double> stationLevels) {
        ...
        StationMeta yMeta = stationMetaMapper.findById("yangpo");
        StationMeta hMeta = stationMetaMapper.findById("hoguk");

        try {
            if (geminiApiKey != null && !geminiApiKey.isBlank()) {
                return callGeminiApi(yangpo, hoguk, yMeta, hMeta);
            }
        } catch (Exception e) {
        ...
        return AiGuideResponse.builder()
                .message(generateRuleBasedMessage(yangpo, hoguk, yMeta, hMeta))
    ...
    private AiGuideResponse callGeminiApi(double yangpo, double hoguk,
                                  StationMeta yMeta, StationMeta hMeta) throws Exception {
        String yStatus = calcStatus(yangpo, yMeta);
        String hStatus = calcStatus(hoguk,  hMeta);
        double yDiff = yangpo - yMeta.getAttwl();
        double hDiff = hoguk  - hMeta.getAttwl();

        String prompt =
            "낙동강 실시간 수위 현황을 바탕으로 강변 여가객에게 상세한 안전 브리핑을 작성해줘.\n\n" +
            "[양포교 — 구미시]\n" +
            "현재: " + String.format("%.2f", yangpo) + "m | 단계: " + yStatus + " | 체감: " + heightLabel(yangpo) + "\n" +
            "기준: 관심 " + yMeta.getAttwl() + "m / 주의보 " + yMeta.getWrnwl() + "m / 경보 " + yMeta.getAlmwl() + "m / 심각 " + yMeta.getSrswl() + "m\n" +
            "관심수위 대비: " + String.format("%+.2f", yDiff) + "m\n\n" +
            "[호국의다리 — 칠곡군]\n" +
            "현재: " + String.format("%.2f", hoguk) + "m | 단계: " + hStatus + " | 체감: " + heightLabel(hoguk) + "\n" +
            "기준: 관심 " + hMeta.getAttwl() + "m / 주의보 " + hMeta.getWrnwl() + "m / 경보 " + hMeta.getAlmwl() + "m / 심각 " + hMeta.getSrswl() + "m\n" +
        ...
        java.util.Map<String, Boolean> activities = new java.util.LinkedHashMap<>();
        ...
        return AiGuideResponse.builder()
                ...
                .generatedAt(java.time.LocalDateTime.now().format(java.time.format.DateTimeFormatter.ISO_LOCAL_DATE_TIME))
                .cached(false)
                .build();
    }
    ...
    private String generateRuleBasedMessage(double yangpo, double hoguk,
                                             StationMeta yMeta, StationMeta hMeta) {
        String yStatus = calcStatus(yangpo, yMeta);
        String hStatus = calcStatus(hoguk,  hMeta);
        boolean anyCritical = "critical".equals(yStatus) || "critical".equals(hStatus);
        boolean anyWarning  = "warning".equals(yStatus)  || "warning".equals(hStatus);
        boolean anyCaution  = "caution".equals(yStatus)  || "caution".equals(hStatus);
        double yDiff = yangpo - yMeta.getAttwl();
        double hDiff = hoguk  - hMeta.getAttwl();

        String yBody  = heightLabel(yangpo);
        String hBody  = heightLabel(hoguk);
        ...
        if (anyCritical) {
            return String.format(
                ...
                yangpo, yDiff, yBody, hoguk, hDiff, hBody);
        } else if (anyWarning) {
            return String.format(
                "🔴 낙동강 경계 단계 발령. 양포교 %.2fm(관심수위 대비 %+.2fm, %s), " +
    ...
    private String calcStatus(double level, StationMeta meta) {
        if (meta == null) return "normal";
        if (level >= meta.getSrswl()) return "critical";
        if (level >= meta.getAlmwl()) return "warning";
        if (level >= meta.getWrnwl()) return "caution";
        return "normal";
    }
}
